...
export class UploadService {
  private apiUrl = 'http://localhost:3000/firebase/upload';
  ...
  uploadImage(file: File): Observable<any> {
    ...
    return this.http.post(`${this.apiUrl}/addimg`, formData, {
      reportProgress: true,
      observe: 'events',
    });
  }
  ...
  getImage(fileName: string): Observable<any> {
    return this.http.get(`${this.apiUrl}/getimg/${fileName}`);
  }

  // Método para eliminar una imagen por nombre de archivo
  deleteImage(fileName: string): Observable<any> {
    return this.http.delete(`${this.apiUrl}/deleteimg/${fileName}`);
  }
}
